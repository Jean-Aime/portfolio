...
const Projects = () => {
  ...
  return (
    <section id="projects" className="py-12 sm:py-16 relative bg-[#020617] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-10 gap-6">
          <div>
            ...
            <p className="text-slate-400 max-w-2xl text-xs sm:text-sm mt-2">
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            className="p-4 rounded-2xl bg-slate-900/80 border border-white/10 backdrop-blur-xl flex items-center gap-4 shrink-0 hover:border-primary/40 transition-colors"
          >
            <div className="size-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-primary">
              <Github size={20} />
            </div>
            <div>
              ...
              <p className="text-[11px] text-slate-400 font-mono mt-0.5">Jean-Aimé GitHub Overview</p>
            </div>
            <a 
              href="https://github.com/Jean-Aime" 
              target="_blank" 
              rel="noopener noreferrer"
              className="p-2 rounded-xl bg-white/5 text-slate-400 hover:text-white hover:bg-primary/20 transition-all ml-1"
          ...
          {projects.map((proj) => {
            ...
            return (
              <button
                ...
                className={`group relative p-5 rounded-2xl border transition-all duration-300 text-left backdrop-blur-xl ${
                  isActive
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className={`p-2.5 rounded-xl transition-colors ${isActive ? "bg-primary text-white" : "bg-white/5 text-primary"}`}>
                    {proj.icon}
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="grid lg:grid-cols-12 gap-0">
              ...
              <div className="lg:col-span-7 relative min-h-[300px] sm:min-h-[380px] bg-slate-950 overflow-hidden group">
                <img 
                ...
                <div className="absolute top-5 left-5 right-5 flex items-center justify-between">
                  ...
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-950/80 border border-white/10 text-slate-300 text-[10px] font-mono">
                    <ShieldCheck size={12} className="text-primary" /> Verified Production
                  </div>
                </div>

                <div className="absolute bottom-5 left-5 right-5 p-4 rounded-2xl bg-black/70 backdrop-blur-xl border border-white/10">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-white font-bold text-sm sm:text-base">{currentProject.name}</h4>
              ...
              <div className="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-between space-y-6 border-t lg:border-t-0 lg:border-l border-white/10">
                <div className="space-y-5">
                  <div>
                    <div className="flex items-center gap-2 text-primary text-xs font-mono font-bold uppercase tracking-widest mb-1.5">
                      <Layers size={13} /> Overview & Mission
                    </div>
                  ...
                  <div>
                    <h5 className="text-xs font-mono uppercase text-slate-400 tracking-wider mb-2 flex items-center gap-1.5">
                      <Code2 size={12} className="text-primary" /> Capabilities & Stack
                    </h5>
                    <div className="flex flex-wrap gap-1.5">
                      {currentProject.techStack.map((tech) => (
                        <span 
                          key={tech} 
                          className="px-2.5 py-0.5 rounded-lg bg-white/5 border border-white/10 text-slate-300 font-mono text-[11px] font-medium"
                        >
                          {tech}
                        </span>
                      ))}
                ...
                <div className="pt-4 border-t border-white/10 grid grid-cols-3 gap-2">
                  {currentProject.metrics.map((m, idx) => (
                    <div key={idx} className="text-center p-2 rounded-xl bg-white/5 border border-white/5">
                      <div className="text-white font-bold font-mono text-xs">{m.value}</div>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
          {projects.map((p) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-3.5 rounded-2xl bg-slate-900/40 border border-white/10 hover:border-primary/40 transition-all flex items-center justify-between group"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs sm:text-sm font-bold text-white group-hover:text-primary transition-colors">
                    {p.name}
                  </div>
              ...
              <ArrowUpRight size={16} className="text-slate-400 group-hover:text-primary group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
            </a>
          ))}
        </div>
