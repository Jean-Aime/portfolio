...
const Contact = () => {
  ...
  const contactMethods = [
    {
      icon: <Mail className="text-sky-400" size={22} />,
      label: "Email",
    ...
    {
      icon: <Linkedin className="text-blue-500" size={22} />,
      label: "LinkedIn",
  ...
  return (
    <section id="contact" className="py-12 sm:py-16 relative overflow-hidden bg-slate-950/40">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-10">
          {contactMethods.map((method, i) => (
            <motion.div
              ...
              className="group relative p-6 rounded-2xl bg-slate-900/60 border border-white/10 hover:border-primary/40 transition-all duration-300 flex flex-col justify-between backdrop-blur-xl"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="size-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    {method.icon}
                  </div>
                  <button
                    onClick={() => handleCopy(method.copyText, i, method.label)}
                    className="p-1.5 rounded-lg bg-white/5 border border-white/10 text-slate-400 hover:text-white hover:bg-primary/20 transition-all"
                    title="Copy info"
                  >
                ...
                <h3 className="text-[10px] font-black text-primary uppercase tracking-widest mb-1">{method.label}</h3>
                <a 
                  href={method.href} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="text-base font-bold text-white hover:text-primary transition-colors block mb-2 truncate"
                >
                  {method.value}
                </a>
              ...
              <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-xs font-bold text-slate-400 group-hover:text-primary transition-colors">
        ...
        <div className="bg-slate-900/80 border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-2xl grid lg:grid-cols-12 gap-8">
          <div className="lg:col-span-5 space-y-5">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 text-[10px] font-mono uppercase tracking-wider">
              <Sparkles size={11} /> Direct Dispatch
            </div>
            <h3 className="text-xl sm:text-2xl font-black text-white">Send a Direct Message</h3>
            ...
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-2.5 font-mono text-xs text-slate-300">
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-7 space-y-3.5">
            <div className="grid sm:grid-cols-2 gap-3.5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:border-primary transition-colors text-xs sm:text-sm"
                />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:border-primary transition-colors text-xs sm:text-sm resize-none"
              />
            </div>
            ...
            <Button
              ...
              className="w-full h-11 bg-primary hover:bg-primary/90 text-white font-bold rounded-xl text-xs sm:text-sm gap-2 shadow-lg shadow-primary/20"
            >
              {isSubmitting ? (
